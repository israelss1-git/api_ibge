...
class ConexaoBD(object):  # classe de conexão com o BD
    ...
    def conexao_azure(
            self):  # aqui é feita a conexão com o bd, podendo ser mysql ou sqlserver, linux ou windows, local ou azure
        ...
        if self.verbose == "sqlserver":  # opção que indica conexão com o sql server
            try:
                ...
                if err.args[0] == "IM002":
                    print(
                        "Não foi possível realizar a conexão com o banco de dados (nome do Driver incorreto). Por favor, reveja o nome do Driver desejado.")  # erro de driver
                elif err.args[0] == "08001":
                    print(
                        "Não foi possível realizar a conexão com o banco de dados (Servidor inexistente ou acesso negado). Por favor, reveja o nome do Servidor desejado.")  # erro de servidor
                elif err.args[0] == "42000":
                    print(
                        "Não foi possível Realizar a conexão com o banco de dados (BD inexistente). Por favor, reveja o nome do BD desejado.")  # erro de banco de dados
                elif err.args[0] == "28000":
                    print(
                        "Não foi possível Realizar a conexão com o banco de dados (Login ou senha invalidos). Por favor, reveja os respectivos campos.")  # erro de login ou senha
                else:
                    print(
                        f"Não foi possível Realizar a conexão com o banco de dados: {err.args[1]}. Por favor, tente novamente. ")  # erro genérico não listado acima
            ...
            try:
                ...
                if err.sqlstate == "42000":
                    ...
                    print(
                        f"Não foi possível Realizar a conexão com o banco de dados: {err.msg}. Por favor, tente novamente. ")  # erro genérico não listado acima
